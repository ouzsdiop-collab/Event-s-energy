  ...
  return (
    <div className="bg-[#0f2d1f] overflow-hidden">
      <div className="relative h-8">
        <svg viewBox="0 0 1440 32" preserveAspectRatio="none" className="absolute inset-0 w-full h-full" style={{ transform: "scaleY(-1)" }}>
      ...
      <div className="max-w-7xl mx-auto px-4 pt-8 pb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-forest-500 mb-1">{p.label}</p>
      ...
      {marqueeItems.length > 0 && (
        <div className="relative pb-10">
          <div className="absolute left-0 top-0 bottom-0 w-24 z-10 pointer-events-none" style={{ background: "linear-gradient(to right, #0f2d1f, transparent)" }} />
          <div className="absolute right-0 top-0 bottom-0 w-24 z-10 pointer-events-none" style={{ background: "linear-gradient(to left, #0f2d1f, transparent)" }} />
          ...
          <div className="marquee-track flex items-center gap-5">
            {marqueeItems.map((partner, i) => (
              ...
              >
                ...
                {partner.logo_url ? (
                  <img src={partner.logo_url} alt={partner.name} className="h-5 w-auto object-contain" />
                ) : (
                  <span
                    className="font-heading font-bold text-sm whitespace-nowrap"
                    style={{
                      color:
                        partner.tier === "platine" ? "#d4aa3a"
                        : partner.tier === "or" ? "#c49a30"
                        : partner.tier === "institutionnel" ? "rgba(255,255,255,0.70)"
