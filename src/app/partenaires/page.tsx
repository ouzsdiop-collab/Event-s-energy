...
const tiers = [
  {
    ...
    color: "#c49a30",
    bgColor: "rgba(196,154,48,0.06)",
    borderColor: "rgba(196,154,48,0.35)",
  ...
  },
  {
    ...
    price: "Sur mesure",
    color: "#246444",
    bgColor: "rgba(36,100,68,0.04)",
    borderColor: "rgba(36,100,68,0.18)",
...
const currentPartners = [
  { name: "UEMOA", tier: "institutionnel", desc: "Union Économique et Monétaire Ouest Africaine" },
  { name: "BAD", tier: "institutionnel", desc: "Banque Africaine de Développement" },
  { name: "BOAD", tier: "institutionnel", desc: "Banque Ouest Africaine de Développement" },
  { name: "TotalEnergies", tier: "platine", desc: "Énergie · Partenaire principal" },
  { name: "Petronas", tier: "platine", desc: "Énergie · Partenaire principal" },
  { name: "Afreximbank", tier: "or", desc: "Banque africaine d'import-export" },
  { name: "IFC", tier: "or", desc: "International Finance Corporation" },
  { name: "NNPC", tier: "or", desc: "Nigerian National Petroleum Corporation" },
  { name: "bp", tier: "or", desc: "British Petroleum" },
  { name: "Société Générale", tier: "argent", desc: "Banque & Services financiers" },
];
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {stats.map((s, i) => (
              <div key={i} className="rounded-xl p-4 text-center" style={{ backgroundColor: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}>
                <div className="font-heading font-black text-2xl mb-1" style={{ background: "linear-gradient(135deg, #c49a30, #e8c96a)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
                  {s.value}
                </div>
                ...
                <div className="bg-white rounded-2xl border shadow-sm overflow-hidden hover:shadow-md transition-all duration-200" style={{ borderColor: tier.borderColor }}>
                  <div className="flex flex-col md:flex-row">
                    ...
                    <div className="md:w-64 shrink-0 p-6 md:p-7 flex flex-col justify-between" style={{ backgroundColor: tier.bgColor, borderRight: `1px solid ${tier.borderColor}` }}>
                      <div>
                        <div className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.15em] px-2.5 py-1 rounded-full mb-4" style={{ backgroundColor: tier.color, color: "#fff" }}>
                          {tier.label}
                        </div>
                        <div className="font-heading font-black text-2xl md:text-3xl mb-1" style={{ color: tier.color }}>
                          {tier.price}
                        </div>
                      ...
                      <div className="grid sm:grid-cols-2 gap-x-6 gap-y-2.5">
                        {tier.avantages.map((av) => (
                          <div key={av} className="flex items-start gap-2 text-xs text-gray-600">
                            <svg className="w-3.5 h-3.5 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{ color: tier.color }}>
            ...
            {currentPartners.map((p, i) => (
              <FadeIn key={p.name} delay={i * 50}>
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex flex-col items-center text-center hover:shadow-md hover:border-gray-200 transition-all duration-200 h-full">
                  ...
                  <p className="font-heading font-bold text-gray-900 text-xs mb-1">{p.name}</p>
                  <p className="text-[10px] text-gray-400 leading-snug mb-2">{p.desc}</p>
            ...
            <div className="px-8 py-10" style={{ background: "linear-gradient(135deg, #0f2d1f, #163d2a)" }}>
              <div className="max-w-2xl mx-auto text-center">
                ...
                <p className="text-white/40 text-sm mb-7 leading-relaxed">
                  Notre équipe est disponible pour étudier avec vous le niveau de partenariat le mieux adapté à vos objectifs et à votre budget. Des formules sur mesure peuvent être envisagées.
                </p>
