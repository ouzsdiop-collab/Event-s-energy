...
import {
  ...
  Bell, ChevronRight, Search, Filter, Download, Eye,
  CheckCircle2, XCircle, Clock, AlertCircle, TrendingUp,
  Globe, Mic, Building2, Send, Menu, X,
} from "lucide-react";
...
const PARTICIPANTS: Participant[] = [
  { id: "P001", nom: "Ouédraogo", prenom: "Kofi", email: "k.ouedraogo@energygh.com", organisation: "Ghana Energy Corp", pays: "Ghana", pass: "Exposant", statut: "confirmé", montant: 2500, methode: "Virement bancaire", date: "2026-11-03" },
  ...
  { id: "P003", nom: "Mensah", prenom: "Kweku", email: "k.mensah@minen.gov.gh", organisation: "Min. Énergie Ghana", pays: "Ghana", pass: "Institutionnel", statut: "VIP", montant: 1200, methode: "Virement bancaire", date: "2026-11-12" },
  { id: "P004", nom: "Traoré", prenom: "Adama", email: "a.traore@sonacos.sn", organisation: "Sonacos Sénégal", pays: "Sénégal", pass: "Professionnel", statut: "confirmé", montant: 1800, methode: "Mobile Money", date: "2026-11-15" },
  { id: "P005", nom: "Bah", prenom: "Ibrahim", email: "i.bah@sogaz.ci", organisation: "SOGAZ Côte d'Ivoire", pays: "Côte d'Ivoire", pass: "Exposant", statut: "en attente", montant: 2500, methode: "Carte bancaire", date: "2026-11-18" },
  { id: "P006", nom: "Sawadogo", prenom: "Rasmata", email: "r.sawadogo@minbf.gov", organisation: "Min. Mines Burkina", pays: "Burkina Faso", pass: "Institutionnel", statut: "confirmé", montant: 1200, methode: "Virement bancaire", date: "2026-11-20" },
  { id: "P007", nom: "Kone", prenom: "Lacina", email: "l.kone@totalci.com", organisation: "TotalEnergies CI", pays: "Côte d'Ivoire", pass: "Exposant", statut: "confirmé", montant: 2500, methode: "Virement bancaire", date: "2026-11-22" },
  { id: "P008", nom: "Sow", prenom: "Fatoumata", email: "f.sow@ong-energie.org", organisation: "ONG Énergie Propre", pays: "Sénégal", pass: "Professionnel", statut: "en attente", montant: 1800, methode: "Mobile Money", date: "2026-11-25" },
  { id: "P009", nom: "Zongo", prenom: "Emmanuel", email: "e.zongo@gazoduc.bf", organisation: "Gazoduc Burkina", pays: "Burkina Faso", pass: "Professionnel", statut: "confirmé", montant: 1800, methode: "Virement bancaire", date: "2026-11-28" },
  ...
  { id: "P011", nom: "Agbodjan", prenom: "Kossi", email: "k.agbodjan@ceet.tg", organisation: "CEET Togo", pays: "Togo", pass: "Institutionnel", statut: "confirmé", montant: 1200, methode: "Virement bancaire", date: "2026-12-03" },
  { id: "P012", nom: "Hounkpatin", prenom: "Séraphine", email: "s.hounkpatin@sogbee.bj", organisation: "SOGBEE Bénin", pays: "Bénin", pass: "Professionnel", statut: "annulé", montant: 1800, methode: "Carte bancaire", date: "2026-12-05" },
  { id: "P013", nom: "Sylla", prenom: "Boubacar", email: "b.sylla@minmines.gn", organisation: "Min. Mines Guinée", pays: "Guinée", pass: "Institutionnel", statut: "en attente", montant: 1200, methode: "Mobile Money", date: "2026-12-08" },
  { id: "P014", nom: "Dembélé", prenom: "Oumar", email: "o.dembele@gazmali.ml", organisation: "GazMali S.A.", pays: "Mali", pass: "Exposant", statut: "confirmé", montant: 2500, methode: "Virement bancaire", date: "2026-12-10" },
...
const TIMELINE = [
  3, 5, 7, 4, 9, 12, 15, 11, 18, 22, 20, 25, 30, 28, 35, 40, 38, 45,
];

const PAYS_DATA = [
  { pays: "Sénégal", count: 42 },
  { pays: "Côte d'Ivoire", count: 38 },
  { pays: "Bénin", count: 35 },
  { pays: "Burkina Faso", count: 28 },
  { pays: "Mali", count: 24 },
  { pays: "Ghana", count: 21 },
  { pays: "Guinée", count: 18 },
  { pays: "Togo", count: 15 },
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex items-start gap-4">
      ...
      <div>
        ...
        <p className="font-heading font-black text-2xl leading-none" style={{ color: "#0f2d1f" }}>{value}</p>
        <p className="text-xs mt-1" style={{ color: "rgba(15,45,31,0.40)" }}>{sub}</p>
      </div>
    </div>
  );
}
...
function SVGBarChart({ data }: { data: number[] }) {
  ...
  const w = 500, h = 120, barW = 18, gap = 9;
  return (
...
function SectionOverview() {
  ...
  const passBreakdown = (["Conférencier","Exposant","Professionnel","Institutionnel"] as PassType[]).map(p => ({
    label: p,
    value: PARTICIPANTS.filter(x => x.pass === p).length,
    color: PASS_COLORS[p],
  }));
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="font-heading font-black text-xl mb-1" style={{ color: "#0f2d1f" }}>Vue d'ensemble</h2>
        <p className="text-xs" style={{ color: "rgba(15,45,31,0.45)" }}>Tableau de bord — données simulées au {new Date().toLocaleDateString("fr-FR")}</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard label="Inscrits totaux" value={String(PARTICIPANTS.length)} sub={`${confirmed} confirmés`} icon={Users} accent="#246444" />
        <KPICard label="Revenus générés" value={fmtMoney(revenue)} sub="hors conférenciers" icon={TrendingUp} accent="#c49a30" />
        <KPICard label="Sessions planifiées" value={String(SESSIONS.length)} sub="sur 3 jours" icon={CalendarDays} accent="#1e5238" />
        <KPICard label="Pays représentés" value={String(PAYS_DATA.length)} sub="dont 1 international" icon={Globe} accent="#0f2d1f" />
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          <p className="text-[10px] mt-2" style={{ color: "rgba(15,45,31,0.35)" }}>Barre dorée = dernière mise à jour</p>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <div className="space-y-3">
          {[...PAYS_DATA].sort((a,b) => b.count - a.count).map((p) => {
            const max = Math.max(...PAYS_DATA.map(x => x.count));
            ...
            return (
              <div key={p.pays} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-2 rounded-full" style={{ backgroundColor: "rgba(36,100,68,0.08)" }}>
                  <div className="h-2 rounded-full transition-all" style={{ width: `${pct}%`, backgroundColor: "#246444" }} />
                </div>
...
function SectionParticipants() {
  ...
  const filtered = useMemo(() => PARTICIPANTS.filter(p => {
    ...
    const matchP = !filterPays || p.pays === filterPays;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h2 className="font-heading font-black text-xl mb-1" style={{ color: "#0f2d1f" }}>Participants</h2>
          <p className="text-xs" style={{ color: "rgba(15,45,31,0.45)" }}>{filtered.length} résultat(s) sur {PARTICIPANTS.length}</p>
        </div>
        <button className="inline-flex items-center gap-2 text-xs font-semibold px-4 py-2 rounded-lg"
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <select value={filterStatut} onChange={e => setFilterStatut(e.target.value as Statut | "")}
          ...
          {(["confirmé","en attente","annulé","VIP"] as Statut[]).map(s => (
              ...
              <tr style={{ backgroundColor: "#f4f7f5" }}>
                {["ID","Participant","Organisation","Pays","Pass","Statut","Montant","Date"].map(h => (
                  <th key={h} className="text-left px-4 py-3 font-semibold"
                    style={{ color: "rgba(15,45,31,0.50)", whiteSpace: "nowrap" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map((p, i) => {
                ...
                return (
                  <tr key={p.id}
                    style={{ borderTop: i > 0 ? "1px solid rgba(36,100,68,0.07)" : undefined }}
                    ...
                    <td className="px-4 py-3">
                      <span className="px-2 py-0.5 rounded-md font-semibold text-[10px]"
                        style={{ backgroundColor: PASS_COLORS[p.pass] + "15", color: PASS_COLORS[p.pass] }}>
                        {p.pass}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 font-semibold" style={{ color: p.montant ? "#0f2d1f" : "rgba(15,45,31,0.35)" }}>
                      {p.montant ? fmtMoney(p.montant) : "—"}
                    </td>
...
function SectionPaiements() {
  ...
  const tarifs: Record<PassType, number> = { "Conférencier": 0, "Exposant": 2500, "Professionnel": 1800, "Institutionnel": 1200 };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-heading font-black text-xl mb-1" style={{ color: "#0f2d1f" }}>Paiements & Revenus</h2>
        <p className="text-xs" style={{ color: "rgba(15,45,31,0.45)" }}>Analyse financière — données simulées</p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <KPICard label="Revenus confirmés" value={fmtMoney(totalRevenue)} sub="paiements reçus" icon={TrendingUp} accent="#246444" />
        <KPICard label="En attente" value={fmtMoney(pending)} sub="à encaisser" icon={Clock} accent="#c49a30" />
        <KPICard label="Taux de conversion" value="80%" sub="inscrits confirmés" icon={CheckCircle2} accent="#1e5238" />
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          <div className="space-y-4">
            {passes.filter(p => tarifs[p] > 0).map(pass => {
              ...
              return (
                <div key={pass}>
                  <div className="flex justify-between mb-1.5">
                    ...
                    <span className="text-xs font-semibold" style={{ color: PASS_COLORS[pass] }}>{fmtMoney(rev)}</span>
                  </div>
                  <div className="h-2 rounded-full" style={{ backgroundColor: "rgba(36,100,68,0.08)" }}>
                    <div className="h-2 rounded-full" style={{ width: `${pct}%`, backgroundColor: PASS_COLORS[pass] }} />
                  </div>
                  <p className="text-[10px] mt-1" style={{ color: "rgba(15,45,31,0.40)" }}>{count} participant(s) · {tarifs[pass]} €/u</p>
                </div>
              );
            })}
          </div>
            ...
            {methods.map(m => {
              ...
              return (
                <div key={m} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-2 rounded-full" style={{ backgroundColor: "rgba(36,100,68,0.08)" }}>
                    <div className="h-2 rounded-full" style={{ width: `${pct}%`, backgroundColor: "#246444" }} />
                  </div>
                  <span className="text-xs font-semibold w-8 text-right" style={{ color: "#0f2d1f" }}>{count}</span>
                </div>
              );
            })}
          </div>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {[...PARTICIPANTS].sort((a,b) => b.date.localeCompare(a.date)).slice(0,8).map((p, i) => {
                const s = STATUT_STYLE[p.statut];
                return (
                  <tr key={p.id} style={{ borderTop: i > 0 ? "1px solid rgba(36,100,68,0.07)" : undefined }}>
                    ...
                    <td className="px-4 py-3 font-semibold" style={{ color: p.montant ? "#0f2d1f" : "rgba(15,45,31,0.35)" }}>
                      {p.montant ? fmtMoney(p.montant) : "Gratuit"}
                    </td>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="font-heading font-black text-xl mb-1" style={{ color: "#0f2d1f" }}>Gestion du Programme</h2>
          <p className="text-xs" style={{ color: "rgba(15,45,31,0.45)" }}>{SESSIONS.length} sessions sur 3 jours</p>
        </div>
        <button className="inline-flex items-center gap-2 text-xs font-semibold px-4 py-2 rounded-lg border"
      ...
      {["J1","J2","J3"].map(jour => (
        <div key={jour} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="divide-y" style={{ borderColor: "rgba(36,100,68,0.07)" }}>
            {SESSIONS.filter(s => s.jour === jour).map(session => {
              ...
              return (
                <div key={session.id} className="px-6 py-4 flex items-start gap-4 hover:bg-[#f9fbfa] transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      <span className="text-xs flex items-center gap-1" style={{ color: "rgba(15,45,31,0.50)" }}>
...
Le Comité d'Organisation SOAFGANG 2027`);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-heading font-black text-xl mb-1" style={{ color: "#0f2d1f" }}>Communications</h2>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {quickActions.map(a => (
          <button key={a.label}
            className="text-left flex items-center gap-4 bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:shadow-md transition-all">
        ...
        <div className="flex border-b" style={{ borderColor: "rgba(36,100,68,0.10)" }}>
          {[["compose","Composer un e-mail"],["sent","Historique des envois"]].map(([id, label]) => (
            <button key={id} onClick={() => setTab(id as "compose"|"sent")}
              ...
              style={{
                color: tab === id ? "#246444" : "rgba(15,45,31,0.45)",
                borderBottom: tab === id ? "2px solid #246444" : "2px solid transparent",
              }}>
              {label}
            </button>
          ))}
        ...
        {tab === "compose" ? (
          <div className="p-6 space-y-4">
            <div>
              ...
              <div className="flex gap-2 flex-wrap">
                {["Tous (15)","Confirmés (11)","En attente (3)","VIP (3)","Exposants (4)"].map(g => (
                  <button key={g} className="text-xs px-3 py-1.5 rounded-lg border font-medium"
            ...
            <div className="flex gap-3">
              <button className="inline-flex items-center gap-2 text-xs font-semibold px-5 py-2.5 rounded-lg"
                style={{ backgroundColor: "#246444", color: "white" }}>
                <Send className="w-3.5 h-3.5" /> Envoyer
              </button>
              <button className="text-xs font-medium px-4 py-2.5 rounded-lg border"
          ...
          <div className="divide-y" style={{ borderColor: "rgba(36,100,68,0.07)" }}>
            {[
              { sujet: "Bienvenue — Confirmation d'inscription", dest: "Tous (15)", date: "2026-12-12", statut: "Envoyé" },
              { sujet: "Rappel paiement — Pass Exposant", dest: "Exposants (4)", date: "2026-12-05", statut: "Envoyé" },
              { sujet: "Programme préliminaire SOAFGANG 2027", dest: "Confirmés (11)", date: "2026-11-28", statut: "Envoyé" },
            ].map((e, i) => (
              <div key={i} className="px-6 py-4 flex items-center gap-4">
                ...
                <span className="text-[10px] px-2 py-0.5 rounded-full font-medium shrink-0"
                  style={{ backgroundColor: "rgba(36,100,68,0.10)", color: "#246444" }}>{e.statut}</span>
              </div>
            ))}
          </div>
        )}
    ...
    <div style={{ backgroundColor: "#f4f7f5", minHeight: "100vh" }}>
      ...
      <div className="w-full px-4 py-3 flex items-center gap-3"
        ...
        <p className="text-xs leading-relaxed" style={{ color: "rgba(255,255,255,0.75)" }}>
          ...
          Ce tableau de bord illustre les fonctionnalités d'administration qui pourraient être développées pour SOAFGANG 2027. Toutes les données affichées sont fictives et à titre démonstratif uniquement.
        </p>
      </div>
          ...
          <div className="flex items-center justify-between px-4 py-5"
            ...
            {sidebarOpen && (
              <div>
                ...
                <p className="text-[10px]" style={{ color: "rgba(255,255,255,0.35)" }}>SOAFGANG 2027</p>
              </div>
            )}
            <button onClick={() => setSidebarOpen(!sidebarOpen)}
              className="p-1.5 rounded-lg hover:bg-white/10 transition-colors ml-auto">
              {sidebarOpen ? <X className="w-4 h-4" style={{ color: "rgba(255,255,255,0.55)" }} />
              ...
              return (
                <button key={id} onClick={() => setActive(id)}
                  ...
                  style={{
                    ...
                    color: isActive ? "#c49a30" : "rgba(255,255,255,0.50)",
                  }}>
                  ...
                  {sidebarOpen && isActive && (
                    <ChevronRight className="w-3 h-3 ml-auto" style={{ color: "#c49a30" }} />
                  )}
                </button>
              );
            })}
          ...
          <div className="p-3 border-t" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-black shrink-0"
                style={{ background: "linear-gradient(135deg, #246444, #c49a30)", color: "white" }}>
                NE
              </div>
              {sidebarOpen && (
